 ...
 * 添加分类
 */
function addBuyer(bodyParam){
    var httpR = new createHttpR(url+'addBuyer','post','text',bodyParam,'callBack');
 ...
 * @param id
 */
function updateBuyer(bodyParam){

    var httpR = new createHttpR(url+'updateBuyer','post','text',bodyParam,'callBack');
    httpR.HttpRequest(function(response){
        var obj = JSON.parse(response);
        var status = obj['status'];
        //var msg = obj['msg'];
...
function deleteBuyer(id){
    var bodyParam={'id':id};
    var httpR = new createHttpR(url+'deleteBuyer','post','text',bodyParam,'callBack');
    httpR.HttpRequest(function(response){
        var obj = JSON.parse(response);
        var status = obj['status'];
        //var msg = obj['msg'];
 ...
 * @param pageSize
 */
function  queryBuyers (searchText,currentPage,pageSize) {
    ...
    if(searchText==null||searchText==''){
        ...
        var bodyParam={'page':currentPage,'size':pageSize,'wechat':'%'+searchText+'%'};
    }

    var httpR = new createHttpR(url+'listBuyer','post','text',bodyParam,'callBack');
    httpR.HttpRequest(function(response){
        var obj = JSON.parse(response);
        var status = obj['status'];
        var msg = obj['msg'];
        if(status=='0'){
            ...
            for(var o in data){
                html+='<tr index='+o+' class="gradeX">\n' +
                    '<td>'+data[o].id+'</td>\n' +
                    '<td style="line-height: 50px;width:50px"><img src="'+data[o].img+'" width="50px" height="50px"></td>\n' +
                    '<td>'+data[o].wechat+'</td>\n' +
                    //'<td>'+data[o].last_login_dt+'</td>\n' +
                    '<td>'+data[o].cd_dt+'</td>\n' ;
                ...
                html+='<td><a class="deleteBuyer" href="" index='+o+' data-toggle="modal" data-target="#delete-box"><span class="label label-info label-mini">删除</span></a></td>\n';
                html+='</tr>';
            }
