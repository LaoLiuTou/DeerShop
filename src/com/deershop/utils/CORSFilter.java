...
public class CORSFilter implements Filter { 
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException, ServletException {
		...
		if(source!=null&&!uri.startsWith("/pages")){
			if(!source.equals("xcx")&&!source.equals("app")){
				
				if( !uri.startsWith("/login")){
					String token = request.getHeader("token");
			        String timesamp = request.getHeader("timesamp");
			        String sign = request.getHeader("sign");
			        String signMd5 = MD5Encryption.getEncryption(token + timesamp+"!@QWASZX");
			...
			StringBuffer msg = new StringBuffer("{\"status\":\"source\",\"msg\":\"未知的请求来源！\"}");
        	response.getWriter().write(msg.toString()); 
		}
